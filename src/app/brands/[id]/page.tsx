...
async function getBrandWithProducts(id: string) {
  const [brandRes, productsRes] = await Promise.all([
    fetch(`https://ecommerce.routemisr.com/api/v1/brands/${id}`),
    fetch(
      `https://ecommerce.routemisr.com/api/v1/products?brand=${id}&limit=50`
    ),
  ]);
...
export default async function BrandProductsPage({
  ...
  const { brand, products } = await getBrandWithProducts(id);
      ...
      <div className="bg-green-500 py-10 px-6 mb-8">
        <div className="container mx-auto">
          ...
          <div className="flex items-center gap-4">
            <div className="bg-white rounded-xl p-2 w-16 h-16 relative flex-shrink-0">
              <Image
                src={brand.image}
                alt={brand.name}
                fill
      ...
      <div className="container mx-auto px-4 pb-10">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex items-center gap-1 bg-purple-100 text-purple-700 text-sm px-3 py-1 rounded-full">
            ...
            <Link href="/brands" className="ml-1 hover:text-purple-900">
              ✕
            </Link>
          </div>
          <Link href="/brands" className="text-sm text-gray-500 hover:underline">
            Clear all
          </Link>
        </div>
