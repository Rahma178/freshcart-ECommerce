...
async function getSubcategoryWithProducts(subId: string) {
  const [subRes, productsRes] = await Promise.all([
    fetch(`https://ecommerce.routemisr.com/api/v1/subcategories/${subId}`),
    fetch(
      `https://ecommerce.routemisr.com/api/v1/products?subcategory=${subId}&limit=50`
    ),
  ]);
  ...
  return {
    subcategory: subData.data as Subcategory,
...
export default async function SubcategoryProductsPage({
  ...
  const { subcategory, products } = await getSubcategoryWithProducts(subId);
      ...
      <div className="bg-green-500 py-10 px-6 mb-8">
        <div className="container mx-auto">
          ...
          <div className="flex items-center gap-2 text-white/80 text-sm mb-4">
            ...
            <span>/</span>
            <Link href={`/shop/${id}`} className="hover:text-white">
      ...
      <div className="container mx-auto px-4 pb-10">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex items-center gap-1 bg-purple-100 text-purple-700 text-sm px-3 py-1 rounded-full">
            ...
            <Link href={`/shop/${id}`} className="ml-1 hover:text-purple-900">✕</Link>
          </div>
          <Link href={`/shop/${id}`} className="text-sm text-gray-500 hover:underline">
            Clear all
          </Link>
        </div>
